...
10. **Publish to the Claude artifact first**, following the artifact tool's read-then-publish
    flow (read the current live version, then publish your merged file over it — it refuses
    a publish that hasn't seen the latest live version).

11. **Sync to the repo.** Convert the page to the standalone-HTML shape the repo expects
    (`<!doctype html><html lang="en"><head><meta charset viewport, meta name="robots"
    content="noindex,nofollow">, <title>...` then the rest of the page, closed with
    `</body></html>` — check `dm/index.html`'s current head for the exact pattern). Write it
    to `dm/index.html`, `git fetch origin main` + merge again (state may have changed since
    step 1), commit with a clear message naming the ring and city list, and push.

12. **Update `automation/dm-progress.json` and push it too**: append the new batch to
    `batches` (today's date, `"source": "auto"`, the ring name, and the list of new
    handles), append those handles to `usedHandles`, remove the trimmed batch's entry from
    `batches` if you trimmed, and set `nextRingIndex` to the next index (wrapping to 0 past
    the end of `rings`).

13. **Do not message the user.** This was explicitly requested to run fully autonomously
    with no per-run check-in — the person will look at the site itself if they want to see
    what changed. Only break this rule if something is fundamentally broken and you cannot
    proceed at all (e.g. the repo is inaccessible) — in that case, a brief note is better
    than silent failure.
